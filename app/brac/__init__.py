...
def require_auth(method):
    ...
    def require_auth_func(*args, **kwargs):
        ...
        if token == service.config.get('SECRET_SHARED_KEY'):
            ...
            user = _get_cached_auth_user(api_user_id)
            if not user:
                service.logger.warn("Authentication failure - User does not exist".format(api_user_id))
            ...
            if service.config['UBIOME_AUTH_HEADER'] in flask.request.headers:
                ...
                try:
                    ...
                    flask.g.user = user_model.User(
                        jwt=flask.request.headers.get(service.config.get('UBIOME_AUTH_HEADER')))

                    user_id = jwt_token.get('user_id')
                    user = _get_cached_auth_user(user_id)

                    if not user:
                        service.logger.warn("Authentication failure - User does not exist".format(user_id))
                        return service.response_class(
                            flask.json.dumps(
                                dict(message="Unauthorized exception - User does not exist".format(user_id))),
                            mimetype='application/json',
                            status=401
                        )
...
def _get_cached_auth_user(user_id):
    return _get_auth_user(user_id)


def _get_auth_user(user_id):
